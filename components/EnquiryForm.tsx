...
export default function EnquiryForm() {
  ...
  return <div className="enquiry-form-wrap">
    <form onSubmit={prepare} className="enquiry-form" onChange={() => { setDraft(null); setStatus(''); }}>
      <div className="form-heading"><span className="eyebrow">Start a conversation</span><h3>How can we help?</h3><p>Prepare an email to the school. You can review it before sending from your email app.</p></div>
      ...
      <label className="field">Preferred visit date <span className="optional">Optional</span><input name="date" type="date" aria-describedby="visit-date-note" /><span id="visit-date-note" className="field-note">The school will confirm availability with you.</span></label>
    ...
    {draft && <div className="enquiry-preview" ref={preview} tabIndex={-1} aria-labelledby="enquiry-preview-title">
      ...
      <div className="draft-actions"><a className="pill-button" href={'mailto:' + school.email + '?subject=' + encodeURIComponent(draft.subject) + '&body=' + encodeURIComponent(draft.body)}>Open email app <ArrowIcon size={17} /></a><button type="button" className="outline-button" onClick={copyDraft}>Copy enquiry</button></div>
